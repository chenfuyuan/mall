...
public class CategoryDO implements Serializable {
	...
	private List<CategoryDO> subCategorys;

	/**
	 * 获取一个排序比较器
	 * @return
	 */
	public static Comparator<CategoryDO> getSortComparator(){
	 ...
	 * @return
	 */
	public static List<CategoryDO> listToTree(List<CategoryDO> source) {
		//过滤出一级分类
		List<CategoryDO> result = source.stream().filter(category ->
				category.getCatLevel() == PmsConstant.CATEGORT_LEVEL_ONE
		).map(category -> {
			//填充子分类
			category.setSubCategorys(buildSubCategorysByList(category, source));
			return category;
		}).sorted(CategoryDO.getSortComparator()).collect(Collectors.toList());
		return result;
	}
	 ...
	 * @return
	 */
	private static List<CategoryDO> buildSubCategorysByList(CategoryDO root, List<CategoryDO> source) {
		if (root == null) {
			return null;
		}
